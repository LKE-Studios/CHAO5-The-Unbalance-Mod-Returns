...
#[status_script( agent = "ike", status = FIGHTER_IKE_STATUS_KIND_SPECIAL_S_ATTACK, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN )]
unsafe fn status_ike_special_s_attack_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT);
    ike_special_s_attack_motion_handler(fighter);
    fighter.sub_shift_status_main(L2CValue::Ptr(ike_special_s_attack_main_loop as *const () as _))
}

unsafe fn ike_special_s_attack_motion_handler(fighter: &mut L2CFighterCommon) {
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT) {
            ...
            MotionModule::change_motion(fighter.module_accessor, Hash40::new("special_air_s_attack"), 0.0, 1.0, false, 0.0, false, false);
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT) {
            ...
            MotionModule::change_motion(fighter.module_accessor, Hash40::new("special_s_attack"), 0.0, 1.0, false, 0.0, false, false);
            WorkModule::on_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT);
        }
    }
}

unsafe extern "C" fn ike_special_s_attack_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
#[status_script( agent = "ike", status = FIGHTER_IKE_STATUS_KIND_SPECIAL_S_END, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN )]
unsafe fn status_ike_special_s_end_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT);
...
unsafe fn ike_special_s_end_motion_handler(fighter: &mut L2CFighterCommon) {
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        fighter.sub_fighter_cliff_check(GROUND_CLIFF_CHECK_KIND_ALWAYS_BOTH_SIDES.into());
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_IKE_STATUS_SPECIAL_S_FLAG_CONTINUE_MOT) {
...
unsafe fn ike_special_s_end_status_handler(fighter: &mut L2CFighterCommon) {
...
pub fn install() {
    install_status_scripts!(
        status_ike_special_s_attack_main,
        status_ike_special_s_end_main
    );
}