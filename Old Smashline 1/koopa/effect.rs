...
unsafe fn effect_koopa_attackairlw(fighter: &mut L2CAgentBase) {
    ...
    WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) <= 15 { //Midbus
        ...
        if is_excute(fighter) {
			EFFECT_FOLLOW_ALPHA(fighter, Hash40::new("sys_attack_impact"), Hash40::new("haver"), 0, 0, 0, 0, 0, 0, 2.0, true, 2);
		}
    } else { //Bowser
        frame(fighter.lua_state_agent, 5.0);
        if is_excute(fighter) {
...
unsafe fn effect_koopa_specialscatch(fighter: &mut L2CAgentBase) {
    ...
    WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) <= 15 { //Midbus
        ...
        if is_excute(fighter) {
            ...
            EFFECT_FOLLOW_COLOR(fighter.lua_state_agent);
        }
        frame(fighter.lua_state_agent, 5.0);
        if is_excute(fighter) {
            EFFECT_FOLLOW_FLIP(fighter, Hash40::new("koopa_scratch"), Hash40::new("koopa_scratch"), Hash40::new("top"), 0, 11, 8, 5, -3, -4, 2.0, true, *EF_FLIP_YZ);
            LAST_EFFECT_SET_COLOR(fighter, /*R*/ 1.8, /*G*/ 0.17, /*B*/ 1.06);
        }
        frame(fighter.lua_state_agent, 8.0);
        if is_excute(fighter) {
...
unsafe fn effect_koopa_speciallw(fighter: &mut L2CAgentBase) {
    ...
    WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) <= 15 { //Midbus
        ...
        if is_excute(fighter) {
            EFFECT_FOLLOW_ALPHA(fighter, Hash40::new("sys_attack_impact"), Hash40::new("haver"), 0, 0, 0, 0, 0, 0, 2.0, true, 2);
            FOOT_EFFECT(fighter, Hash40::new("sys_turn_smoke"), Hash40::new("top"), -2.0, 0, 0, 0, 0, 0, 2.0, 0, 0, 0, 0, 0, 0, false);
        }
        for _ in 0..6 {
            if is_excute(fighter) {
                FLASH(fighter, 0, 0, 0, 0);
