...
unsafe fn status_silver_special_n_shoot_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
#[status_script(agent = "mewtwo", status = FIGHTER_STATUS_KIND_SPECIAL_LW, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_PRE)]
unsafe fn status_silver_special_lw_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
	let fighter_kind = utility::get_kind(&mut *fighter.module_accessor);
    let SILVER = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) >= 128 && WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) <= 135;
    if SILVER && fighter_kind == *FIGHTER_KIND_MEWTWO {
        StatusModule::init_settings(fighter.module_accessor, SituationKind(*SITUATION_KIND_NONE), *FIGHTER_KINETIC_TYPE_NONE, *GROUND_CORRECT_KIND_KEEP as u32, GroundCliffCheckKind(*GROUND_CLIFF_CHECK_KIND_NONE), true, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLAG, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_INT, *FIGHTER_STATUS_WORK_KEEP_FLAG_NONE_FLOAT, 0);
...
pub fn install() {
    install_status_scripts!(
        status_silver_special_n_shoot_pre,
        status_silver_special_lw_pre
    );
}