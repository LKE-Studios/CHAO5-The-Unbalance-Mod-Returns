...
unsafe extern "C" fn status_bandana_spear_Stick_Init(weapon: &mut L2CWeaponCommon) -> L2CValue {
...
unsafe extern "C" fn status_bandana_spear_Stick_Main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if !StopModule::is_stop(weapon.module_accessor) {
        bandana_spear_Stick_SubStatus(weapon, false.into());
    }
    weapon.global_table[SUB_STATUS].assign(&L2CValue::Ptr(bandana_spear_Stick_SubStatus as *const () as _));
    weapon.fastshift(L2CValue::Ptr(bandana_spear_Stick_Main_loop as *const () as _))
}

unsafe extern "C" fn bandana_spear_Stick_SubStatus(weapon: &mut L2CWeaponCommon, life_dec: L2CValue) -> L2CValue {
...
unsafe extern "C" fn bandana_spear_Stick_Main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
...
unsafe extern "C" fn status_bandana_spear_Stick_End(weapon: &mut L2CWeaponCommon) -> L2CValue {
    0.into()
}

pub fn install() {
    Agent::new("edge_spear")
    .status(Pre, *WN_LINK_BOWARROW_STATUS_KIND_STICK, status_bandana_spear_Stick_Pre)
    .status(Init, *WN_LINK_BOWARROW_STATUS_KIND_STICK, status_bandana_spear_Stick_Init)
    .status(Main, *WN_LINK_BOWARROW_STATUS_KIND_STICK, status_bandana_spear_Stick_Main)
    .status(End, *WN_LINK_BOWARROW_STATUS_KIND_STICK, status_bandana_spear_Stick_End)
    .install();
}