...
unsafe extern "C" fn status_bandana_spear3_Fly_Init(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let owner_module_accessor = &mut *sv_battle_object::module_accessor((WorkModule::get_int(weapon.module_accessor, *WEAPON_INSTANCE_WORK_ID_INT_LINK_OWNER)) as u32);
    let lr = PostureModule::lr(owner_module_accessor);
    let kind = utility::get_kind(&mut *weapon.module_accessor);
...
unsafe extern "C" fn status_bandana_spear3_Fly_Main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    weapon.fastshift(L2CValue::Ptr(bandana_spear3_Fly_Main_loop as *const () as _))
}


unsafe extern "C" fn bandana_spear3_Fly_Main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
...
unsafe extern "C" fn status_bandana_spear3_Fly_End(weapon: &mut L2CWeaponCommon) -> L2CValue {
    0.into()
}

pub fn install() {
    Agent::new("edge_spear3")
    .status(Pre, *WN_LINK_BOWARROW_STATUS_KIND_FLY, status_bandana_spear3_Fly_Pre)
    .status(Init, *WN_LINK_BOWARROW_STATUS_KIND_FLY, status_bandana_spear3_Fly_Init)
    .status(Main, *WN_LINK_BOWARROW_STATUS_KIND_FLY, status_bandana_spear3_Fly_Main)
    .status(End, *WN_LINK_BOWARROW_STATUS_KIND_FLY, status_bandana_spear3_Fly_End)
    .install();
}