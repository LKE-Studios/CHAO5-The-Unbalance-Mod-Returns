...
unsafe extern "C" fn status_sans_AttackLw3_CheckAttack(fighter: &mut L2CFighterCommon, _param_2: &L2CValue, param_3: &L2CValue) -> L2CValue {
    ...
    if category == *BATTLE_OBJECT_CATEGORY_FIGHTER {
        if SANS && utility::get_kind(module_accessor) == *FIGHTER_KIND_PALUTENA {
            if collision_kind == *COLLISION_KIND_HIT {
                if [hash40("attack_lw3")].contains(&MotionModule::motion_kind(module_accessor)) {
...
pub fn install() {
    Agent::new("palutena")
    .status(CheckAttack, *FIGHTER_STATUS_KIND_ATTACK_LW3, status_sans_AttackLw3_CheckAttack)
    .install();
}