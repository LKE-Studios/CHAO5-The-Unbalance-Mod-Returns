...
static ANGLE_MAX_UP : f32 = 60.0; //Max Ascent Angle for Glide (degrees)
static ANGLE_MAX_DOWN : f32 = -60.0; //Max Descent Angle for Glide (degrees)
static mut MOMENTUM : [f32; 8] = [0.0; 8];
static THRESHOLD_MAX : f32 = -20.0;
static mut ANGLE_FRAME : [f32; 8] = [90.0; 8];
static DIRECTION_UP : f32 = 30.0;
static DIRECTION_DOWN : f32 = 150.0;
static STICK_ANGLE_MUL : f32 = 5.0; //Max Angular Speed
static UP_ANGLE_ACCEL : f32 = 0.75; //Upward angular acceleration
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    macros::SET_SPEED_EX(fighter, 1.7, -0.52, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN); //Base horizontal air mobility and normal descent speed.
    static Y_ACCEL_ADD : f32 = 0.0434; //Ascent/Descent Speed Multiplier
    ...
    if ANGLE[ENTRY_ID] <= 60.0 && ANGLE[ENTRY_ID] >= -60.0 {
    ...
    if ANGLE[ENTRY_ID] >= -60.0 && ANGLE[ENTRY_ID] < -20.0 { //Applies the H Air decel. multilplier when descending when angle is between -60 and -20
    ...
    if ANGLE[ENTRY_ID] >= -20.0 && ANGLE[ENTRY_ID] < 0.0 { //Applies the H Air accel. multilplier when descending when angle is between -20 and 0
        macros::SET_SPEED_EX(fighter, 1.7 + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
        MotionModule::set_rate_partial(fighter.module_accessor, *FIGHTER_METAKNIGHT_MOTION_PART_SET_KIND_WING, 1.0 + ANGLE[ENTRY_ID] * 0.005);
    };
    if ANGLE[ENTRY_ID] <= 60.0 && ANGLE[ENTRY_ID] > 0.0 { //Applies the H Air decel. multilplier when descending when angle is between 0 and 60
        macros::SET_SPEED_EX(fighter, 1.7 + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
