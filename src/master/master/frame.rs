...
pub unsafe extern "C" fn frame_master_Main(fighter : &mut L2CFighterCommon) {
...
pub unsafe extern "C" fn frame_master_spear_Exec(weapon : &mut L2CFighterBase) {
    let status_kind = StatusModule::status_kind(weapon.module_accessor);
    if status_kind == *FIGHTER_STATUS_KIND_SPECIAL_S || status_kind == *FIGHTER_MASTER_STATUS_KIND_SPECIAL_S_FRONT
    || status_kind == *FIGHTER_MASTER_STATUS_KIND_SPECIAL_S_FRONT_DASH {
        ModelModule::set_joint_scale(weapon.module_accessor, Hash40::new("haver"), &Vector3f{x: 2.0, y: 2.0, z: 2.0});
        AttackModule::set_attack_scale(weapon.module_accessor, 1.9, true);
        if MotionModule::frame(weapon.module_accessor) > 52.0 {
            ModelModule::set_joint_scale(weapon.module_accessor, Hash40::new("haver"), &Vector3f{x: 1.0, y: 1.0, z: 1.0});
            AttackModule::set_attack_scale(weapon.module_accessor, 1.0, true);
        }
    }
    else {
        ModelModule::set_joint_scale(weapon.module_accessor, Hash40::new("haver"), &Vector3f{x: 1.0, y: 1.0, z: 1.0});
...
pub unsafe extern "C" fn frame_master_axe_Main(weapon : &mut L2CFighterBase) {
    let status_kind = StatusModule::status_kind(weapon.module_accessor);
...
pub fn install() {
    Agent::new("master")
    .on_line(Main, frame_master_Main)
    .install();
    
    Agent::new("master_axe")
    .on_line(Main, frame_master_axe_Main)
    .install();

    Agent::new("master_spear")
    .on_line(Exec, frame_master_spear_Exec)
    .install();
}