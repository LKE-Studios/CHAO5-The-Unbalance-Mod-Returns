...
static DIRECTION_UP : f32 = 35.0;
static DIRECTION_DOWN : f32 = 145.0;
static STICK_ANGLE_MUL : f32 = 4.5; //Max Angular Speed
static UP_ANGLE_ACCEL : f32 = 0.83; //Upward angular acceleration
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    macros::SET_SPEED_EX(fighter, 1.75, -0.5, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN); //Base horizontal air mobility and normal descent speed.
    static Y_ACCEL_ADD : f32 = 0.045; //Ascent/Descent Speed Multiplier
    static Y_ACCEL_UP : f32 = -0.007;
    static X_DECEL_MUL_UP : f32 = -0.019; //Horizontal Air Deceleration multiplier when ascending in between higher angle values
    ...
    if ANGLE[ENTRY_ID] <= 55.0 && ANGLE[ENTRY_ID] >= -55.0 {
    ...
    if ANGLE[ENTRY_ID] >= -55.0 && ANGLE[ENTRY_ID] < -20.0 { //Applies the H Air decel. multilplier when descending when angle is between -50 and -35.1
    ...
    if ANGLE[ENTRY_ID] >= -20.0 && ANGLE[ENTRY_ID] < 0.0 { //Applies the H Air accel. multilplier when descending when angle is between -15 and 0.1
        macros::SET_SPEED_EX(fighter, 1.75 + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
        MotionModule::set_rate_partial(fighter.module_accessor, *FIGHTER_METAKNIGHT_MOTION_PART_SET_KIND_WING, 1.0 + ANGLE[ENTRY_ID] * 0.01);
    };        
    if ANGLE[ENTRY_ID] <= 55.0 && ANGLE[ENTRY_ID] > 0.0 { //Applies the H Air decel. multilplier when descending when angle is between 30.1 and 50
        macros::SET_SPEED_EX(fighter, 1.75 + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
