...
static GLIDE_START_H_SPEED : f32 = 1.2; //H speed multiplier on GlideStart
static ANGLE_MAX_UP : f32 = 55.0; //Max Ascent Angle for Glide (degrees)
static ANGLE_MAX_DOWN : f32 = -55.0; //Max Descent Angle for Glide (degrees)
static mut MOMENTUM : [f32; 8] = [0.0; 8];
static ANGLE_MAX_SPEED : f32 = -20.0;
static BASE_H_SPEED : f32 = 1.75; //Base H Speed forward on glide
static GRAVITY : f32 = -0.5; //Default descent speed when unangled
static DOWN_H_SPEED : f32 = 2.303; //Sorta Emulates how Brawl handled gaining additional forward speed
static Y_ACCEL_ADD : f32 = 0.045; //Vertical Speed Multiplier
static X_DECEL_MUL_UP : f32 = -0.0174446; 
static X_ACCEL_MUL_DOWN : f32 = -0.025; 
static X_DECEL_MUL_DOWN : f32 =  0.0275; 
static ANGLE_SPEED_MAX : f32 = 4.5; //Max Angular Speed
static mut UP_ANGLE_ACCEL_MIN : [f32; 8] = [0.54; 8]; //Min Upward angular acceleration 
static UP_ANGLE_ACCEL_MAX : f32 = 0.86; //Max Upward angular acceleration
static mut DOWN_ANGLE_ACCEL_MIN : [f32; 8] = [0.6; 8]; //Min Upward angular acceleration 
static DOWN_ANGLE_ACCEL_MAX : f32 = 1.0; //Max Downward angular acceleration

#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE_START, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
...
#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN {
        ...
        SoundModule::set_se_pitch_ratio(fighter.module_accessor, Hash40::new("se_buddy_glide_loop"), 0.94 + ANGLE[ENTRY_ID] * -0.005);
        SoundModule::set_se_pitch_ratio(fighter.module_accessor, Hash40::new("se_buddy_wing"), 1.0 + ANGLE[ENTRY_ID] * 0.0048);
    }
    //Forward Speed Stuff
    if KineticModule::get_kinetic_type(boma) != *FIGHTER_KINETIC_TYPE_MOTION_AIR {
        KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_MOTION_AIR);
    };
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN && ANGLE[ENTRY_ID] < ANGLE_MAX_SPEED { //Applies the H Air decel. multilplier when descending when angle is between -50 and -35.1
    ...
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_SPEED && ANGLE[ENTRY_ID] < 0.0 { //Applies the H Air accel. multilplier when descending when angle is between -15 and 0.1
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
        MotionModule::set_rate_partial(fighter.module_accessor, *FIGHTER_METAKNIGHT_MOTION_PART_SET_KIND_WING, 1.0 + ANGLE[ENTRY_ID] * 0.01);
    };        
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] > 0.0 { //Applies the H Air decel. multilplier when descending when angle is between 30.1 and 50
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
        KineticModule::add_speed(fighter.module_accessor, &Vector3f{x: ANGLE[ENTRY_ID] * X_DECEL_MUL_UP, y:0.0, z:0.0});
        MotionModule::set_rate_partial(fighter.module_accessor, *FIGHTER_METAKNIGHT_MOTION_PART_SET_KIND_WING, 1.0 + ANGLE[ENTRY_ID] * 0.018);
    };
    if speed_x * PostureModule::lr(fighter.module_accessor) < 1.29 {
...
#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_END)]
...
#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE_ATTACK, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
...
#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE_END, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
...
#[status_script(agent = "buddy", status = FIGHTER_STATUS_KIND_GLIDE_LANDING, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
