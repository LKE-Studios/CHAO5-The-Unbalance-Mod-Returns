...
pub static special_hi_barrel_speed : f32 = 4.0;
	...
	if FUNKY {
        ...
        if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FUNKY_STATUS_SPECIAL_HI_FLAG_GROUND_END) {
            if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_FUNKY_STATUS_SPECIAL_HI_FLAG_LAUNCH) {
                ...
                let lr = PostureModule::lr(fighter.module_accessor);
                sv_kinetic_energy!(set_stable_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, speed_x * lr, (speed_y - 0.07).max(min_y));
                sv_kinetic_energy!(set_limit_speed, fighter, *FIGHTER_KINETIC_ENERGY_ID_STOP, speed_x * lr, (speed_y - 0.07).max(min_y));
                ...
                if frame < 10.0 {
                    if can_launch {
                        ...
                        if ControlModule::get_command_flag_cat(fighter.module_accessor, 0) & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_ANY != 0 {
                            ...
                            let angle = barrel_frame - 45.0;
                            let mut barrel_angle = WorkModule::get_float(fighter.module_accessor, *FIGHTER_FUNKY_STATUS_SPECIAL_HI_WORK_FLOAT_BARREL_ANGLE);
                            WorkModule::set_float(fighter.module_accessor, barrel_angle, *FIGHTER_FUNKY_STATUS_SPECIAL_HI_WORK_FLOAT_BARREL_ANGLE);
                            PostureModule::set_rot(barrel_modules, &Vector3f{x: angle, y: 0.0, z: 0.0}, 0);
                        }
                    }
