...
pub unsafe extern "C" fn status_funky_SpecialLwJump_Pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub unsafe extern "C" fn status_funky_SpecialLwJump_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    let color = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR);     
    let FUNKY = color >= 120 && color <= 127;
	if FUNKY {
        ...
        fighter.sub_shift_status_main(L2CValue::Ptr(funky_SpecialLwJump_Main_loop as *const () as _))
    }
    else {
        0.into()
    }
...
unsafe extern "C" fn funky_SpecialLwJump_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub unsafe extern "C" fn status_funky_SpecialLwJump_End(fighter: &mut L2CFighterCommon) -> L2CValue {
    let color = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR);     
    let FUNKY = color >= 120 && color <= 127;
	if FUNKY {
        0.into()
...
pub fn install() {
    Agent::new("donkey")
    .status(Pre, *FIGHTER_FUNKY_STATUS_KIND_SPECIAL_LW_JUMP, status_funky_SpecialLwJump_Pre)
    .status(Main, *FIGHTER_FUNKY_STATUS_KIND_SPECIAL_LW_JUMP, status_funky_SpecialLwJump_Main)
    .status(End, *FIGHTER_FUNKY_STATUS_KIND_SPECIAL_LW_JUMP, status_funky_SpecialLwJump_End)    
    .install();
}