...
unsafe extern "C" fn status_cloud_SpecialHi2Fall_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn cloud_SpecialHi2Fall_Sub_Status(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if param_1.get_bool() {
...
unsafe extern "C" fn cloud_SpecialHi2Fall_handler(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let object_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_CLOUD_STATUS_SPECIAL_HI2_INT_HIT_OBJECT_ID) as u32;
    if -1 < (object_num - 1) {
                ...
                if BattleObjectManager::is_active_find_battle_object(BattleObjectManager(), object_id) {
                    ...
                    if ![*FIGHTER_STATUS_KIND_DAMAGE_FLY, *FIGHTER_STATUS_KIND_DAMAGE_FLY_ROLL, *FIGHTER_STATUS_KIND_DAMAGE_FLY_METEOR].contains(&status_kind) {
                        let transition_terms = [
                            *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE_WALL_JUMP_BUTTON, *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE_WALL_JUMP, *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE_WALL, 
                            *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE_CEIL, *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE_FB, *FIGHTER_STATUS_TRANSITION_TERM_ID_PASSIVE
                        ];
                        for x in 0..transition_terms.len() {
                            WorkModule::enable_transition_term(module_accessor, transition_terms[x]);
                        }
                    }
                }
            }
...
pub fn install() {
    Agent::new("cloud")
    .status(Main, *FIGHTER_CLOUD_STATUS_KIND_SPECIAL_HI2_FALL, status_cloud_SpecialHi2Fall_Main)
    .install();
}