...
static ANGLE_LOW_MAX : f32 = -55.0; //Max Descent Angle for Glide (degrees)
static STICK_ANGLE_MUL : f32 = 4.5; //Controls how much Charizard's body rotates according to the control stick (higher value = higher sensitivity)
...
unsafe extern "C" fn glide_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if stick_y >= 0.1 || stick_y <= -0.1 { //Used to prevent having a stick_y in the middle from changing flight angle
        ...
        if ANGLE[ENTRY_ID] < ANGLE_LOW_MAX {
            ANGLE[ENTRY_ID] = ANGLE_LOW_MAX; //Caps the max downward value at -55 and prevents it from going beyond. 
        };
    };
    ...
    if ANGLE[ENTRY_ID] >= -55.0 && ANGLE[ENTRY_ID] <= -0.1 { //Applies the H Air decel. multilplier when descending when angle is between -55 and 0.1
