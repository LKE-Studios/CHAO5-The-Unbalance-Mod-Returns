...
static GLIDE_START_H_SPEED : f32 = 0.0; //H speed multiplier on GlideStart
static ANGLE_MAX_UP : f32 = 50.0; //Max Upward Angle for Glide (degrees)
static ANGLE_MAX_DOWN : f32 = -50.0; //Max Downward Angle for Glide (degrees)
static mut MOMENTUM : [f32; 8] = [0.0; 8];
static ANGLE_MAX_SPEED : f32 = -20.0;
static BASE_H_SPEED : f32 = 1.5; //Base H Speed forward on glide
static GRAVITY : f32 = -0.6; //Default descent speed when unangled
static DOWN_H_SPEED : f32 = 2.01; //Sorta Emulates how Brawl handled gaining additional forward speed
static Y_ACCEL_ADD : f32 = 0.05; //Ascent/Descent Speed Multiplier
static Y_ACCEL_UP : f32 = -0.02;
static X_DECEL_MUL_UP : f32 = -0.015003;
static X_ACCEL_MUL_DOWN : f32 = -0.03;
static X_DECEL_MUL_DOWN : f32 = 0.02523;
static ANGLE_SPEED_MAX : f32 = 4.0; //Max Angular Speed
static mut UP_ANGLE_ACCEL_MIN : [f32; 8] = [0.67; 8]; //Min Upward angular acceleration 
static UP_ANGLE_ACCEL_MAX : f32 = 0.8; //Max Upward angular acceleration
static mut DOWN_ANGLE_ACCEL_MIN : [f32; 8] = [0.35; 8]; //Min Downward angular acceleration 
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if stick_y > 0.0 { //Used to prevent having a stick_y in the middle from changing flight angle
        ...
        if ANGLE[ENTRY_ID] > ANGLE_MAX_UP { //Caps the max upward value at 50 and prevents it from going beyond. 
    ...
    if stick_y < 0.0 {
        ...
        if ANGLE[ENTRY_ID] < ANGLE_MAX_DOWN {
            ANGLE[ENTRY_ID] = ANGLE_MAX_DOWN; //Caps the max downward value at -50 and prevents it from going beyond. 
            MOMENTUM[ENTRY_ID] = ANGLE_MAX_SPEED;
    ...
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN {
        ...
        SoundModule::set_se_pitch_ratio(fighter.module_accessor, Hash40::new("se_plizardon_glide_loop"), 0.85 + ANGLE[ENTRY_ID] * -0.006);
    }
    //Forward Speed Stuff
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN && ANGLE[ENTRY_ID] < ANGLE_MAX_SPEED {
    ...
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_SPEED && ANGLE[ENTRY_ID] < 0.0 { 
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    };        
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] > 0.0 {
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    ...
    if speed_x * PostureModule::lr(fighter.module_accessor) < 1.34 {
