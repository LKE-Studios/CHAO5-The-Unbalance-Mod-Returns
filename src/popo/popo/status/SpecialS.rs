...
unsafe extern "C" fn status_popo_SpecialS_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    popo_SpecialS_motion_function(fighter, true.into());
    if !StopModule::is_stop(fighter.module_accessor) {
        WorkModule::inc_int(fighter.module_accessor, *FIGHTER_POPO_STATUS_SPECIAL_S_WORK_INT_SPEED_UP_Y_COUNT);
    }
    fighter.global_table[PREV_SUB_STATUS].assign(&L2CValue::Ptr(popo_SpecialS_Prev_Sub_status as *const () as _));
    fighter.sub_shift_status_main(L2CValue::Ptr(popo_SpecialS_Main_loop as *const () as _))
}

unsafe extern "C" fn popo_SpecialS_Prev_Sub_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::inc_int(fighter.module_accessor, *FIGHTER_POPO_STATUS_SPECIAL_S_WORK_INT_SPEED_UP_Y_COUNT);
    0.into()
}

unsafe extern "C" fn popo_SpecialS_motion_function(fighter: &mut L2CFighterCommon, param_1: bool) -> L2CValue {
...
unsafe extern "C" fn popo_SpecialS_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
            ...
            if !StatusModule::is_changing(fighter.module_accessor) {
                if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_FLAG_SUB_FIGHTER) {
                    if LinkModule::is_link(fighter.module_accessor, *FIGHTER_POPO_LINK_NO_PARTNER) {
                        if popo_SpecialS_motion_function(fighter, false.into()).get_bool() {
...
pub fn install() {
    Agent::new("popo")
    .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_S, status_popo_SpecialS_Main)
    .install();
}