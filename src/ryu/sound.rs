...
#[acmd_script(//SpecialAirN
    ...
    category = ACMD_SOUND,
    low_priority )]
unsafe fn sound_neutralbair(fighter: &mut L2CAgentBase) {
    ...
    if macros::is_excute(fighter) {
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_RYU_STATUS_WORK_ID_SPECIAL_COMMON_FLAG_COMMAND) == false {
            if macros::is_excute(fighter) {
                macros::PLAY_SE(fighter, Hash40::new("se_ryu_special_n04"));
            }
            wait(fighter.lua_state_agent, 10.0);
            ...
            if WorkModule::get_int64(fighter.module_accessor, *FIGHTER_RYU_STATUS_WORK_ID_SPECIAL_N_INT_TYPE) == 0 {
                ...
                if WorkModule::get_int64(fighter.module_accessor, *FIGHTER_RYU_STATUS_WORK_ID_SPECIAL_N_INT_TYPE) == 1 {
                    if macros::is_excute(fighter) {
                        macros::PLAY_SE(fighter, Hash40::new("se_ryu_command_success"));
                        macros::PLAY_SE(fighter, Hash40::new("se_ryu_special_n04"));
                    }
