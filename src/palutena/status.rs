...
static ANGLE_MAX_SPEED : f32 = -25.0;
static BASE_H_SPEED : f32 = 1.7; //Base H Speed forward on glide
static GRAVITY : f32 = -0.5; //Default descent speed when unangled
static DOWN_H_SPEED : f32 = 2.63; //Sorta Emulates how Brawl handled gaining additional forward speed
static Y_ACCEL_ADD : f32 = 0.0334; //Vertical Speed Multiplier
static X_DECEL_MUL_UP : f32 = -0.024706;
static X_ACCEL_MUL_DOWN : f32 = -0.032;
static X_DECEL_MUL_DOWN : f32 = 0.03719;
static ANGLE_SPEED_MAX : f32 = 6.5; //Max Angular Speed
static mut UP_ANGLE_ACCEL_MIN : [f32; 8] = [0.52; 8]; //Min Upward angular acceleration 
static UP_ANGLE_ACCEL_MAX : f32 = 0.8; //Max Upward angular acceleration
static mut DOWN_ANGLE_ACCEL_MIN : [f32; 8] = [0.66; 8]; //Min Downward angular acceleration 
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    //Forward Speed Stuff
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN && ANGLE[ENTRY_ID] < ANGLE_MAX_SPEED { //Applies the H Air decel. multilplier when descending when angle is between -70 and -25.0
    ...
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_SPEED && ANGLE[ENTRY_ID] < 0.0 { //Applies the H Air accel. multilplier when descending when angle is between -25 and 0
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    };
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] > 0.0 { //Applies the H Air decel. multilplier when descending when angle is between 0 and 70
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
        KineticModule::add_speed(fighter.module_accessor, &Vector3f{x: ANGLE [ENTRY_ID] * X_DECEL_MUL_UP, y:0.0, z:0.0});
    };
    if speed_x * PostureModule::lr(fighter.module_accessor) < 0.64 {
