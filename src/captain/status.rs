...
#[status_script( agent = "captain", status = FIGHTER_CAPTAIN_STATUS_KIND_SPECIAL_S_END, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN )]
unsafe fn status_captain_special_s_end_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(captain_special_s_end_main_loop as *const () as _))
}

unsafe extern "C" fn captain_special_s_end_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    install_status_scripts!(
        status_captain_special_s_end_main
    );
}