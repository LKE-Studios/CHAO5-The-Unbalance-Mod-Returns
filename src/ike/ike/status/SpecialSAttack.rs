...
unsafe extern "C" fn status_ike_SpecialSAttack_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(ike_SpecialSAttack_Main_loop as *const () as _))
}

unsafe extern "C" fn ike_special_s_attack_motion_handler(fighter: &mut L2CFighterCommon) {
...
unsafe extern "C" fn ike_SpecialSAttack_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("ike")
    .status(Main, *FIGHTER_IKE_STATUS_KIND_SPECIAL_S_ATTACK, status_ike_SpecialSAttack_Main)
    .install();
}