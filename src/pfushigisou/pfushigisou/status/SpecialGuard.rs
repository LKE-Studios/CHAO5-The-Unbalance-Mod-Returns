...
pub unsafe extern "C" fn status_pfushigisou_SpecialGuard_Pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    FighterStatusModuleImpl::set_fighter_status_data(fighter.module_accessor, false, *FIGHTER_TREADED_KIND_NO_REAC, false, false, false, 0, *FIGHTER_STATUS_ATTR_START_TURN as u32, (*FIGHTER_LOG_MASK_FLAG_ACTION_CATEGORY_ATTACK | *FIGHTER_POWER_UP_ATTACK_BIT_SPECIAL_N) as u32, 0);
    0.into()
}
...
unsafe extern "C" fn pfushigisou_SpecialGuard_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ControlModule::check_button_trigger(fighter.module_accessor, *CONTROL_PAD_BUTTON_JUMP) {
        fighter.change_status(FIGHTER_PFUSHIGISOU_STATUS_KIND_SPECIAL_GUARD_END.into(), true.into());
    }
...
pub fn install() {
    ...
    .status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_GUARD, status_pfushigisou_SpecialGuard_Pre)
    .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_GUARD, status_pfushigisou_SpecialGuard_Main)
    .status(Exec, *FIGHTER_STATUS_KIND_SPECIAL_GUARD, status_pfushigisou_SpecialGuard_Exec)
    .status(End, *FIGHTER_STATUS_KIND_SPECIAL_GUARD, status_pfushigisou_SpecialGuard_End)
    .install();
}