...
unsafe extern "C" fn status_donkey_SpecialHi_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    WorkModule::off_flag(fighter.module_accessor, *FIGHTER_DONKEY_STATUS_SPECIAL_HI_FLAG_YACL_DEFAULT);
    donkey_SpecialHi_motion_function(fighter);
    if !StopModule::is_stop(fighter.module_accessor) {
        donkey_SpecialHi_Sub_Status(fighter, false.into());
    }
    fighter.global_table[SUB_STATUS].assign(&L2CValue::Ptr(donkey_SpecialHi_Sub_Status as *const () as _));
    fighter.sub_shift_status_main(L2CValue::Ptr(donkey_SpecialHi_Main_loop as *const () as _))
}

unsafe extern "C" fn donkey_SpecialHi_Sub_Status(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if !param_1.get_bool() {
...
unsafe extern "C" fn donkey_SpecialHi_motion_function(fighter: &mut L2CFighterCommon) {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_DONKEY_STATUS_SPECIAL_HI_FLAG_MOT_CHANGE) {
            if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_DONKEY_STATUS_SPECIAL_HI_FLAG_GROUND_MOT_FRAME) {
                ...
                MotionModule::change_motion_force_inherit_frame(fighter.module_accessor, Hash40::new("special_hi"), ground_mot_frame, 1.0, 0.0);
...
unsafe extern "C" fn donkey_SpecialHi_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let param_x_spd_max = if GroundModule::is_touch(fighter.module_accessor, *GROUND_TOUCH_FLAG_DOWN as u32) { hash40("x_spd_max_ground") } else { hash40("x_spd_max_air") };
    let x_spd_max = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_hi"), param_x_spd_max);
    let param_x_acl = if GroundModule::is_touch(fighter.module_accessor, *GROUND_TOUCH_FLAG_DOWN as u32) { hash40("x_acl_ground") } else { hash40("x_acl_air") };
    let x_acl = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_hi"), param_x_acl);
    ...
    if !GroundModule::is_touch(fighter.module_accessor, *GROUND_TOUCH_FLAG_DOWN as u32) {
        ...
        sv_kinetic_energy!(controller_set_accel_x_mul, fighter, x_acl);
        sv_kinetic_energy!(set_stable_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, x_spd_max, 0.0);
        sv_kinetic_energy!(set_limit_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, x_spd_max, 0.0);
        ...
        if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_DONKEY_STATUS_SPECIAL_HI_FLAG_GROUND_SPINEND){
            ...
            let x_dcl_ground_spinend = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_hi"), hash40("x_dcl_ground_spinend"));
            sv_kinetic_energy!(set_brake, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, x_dcl_ground_spinend, 0.0);
            sv_kinetic_energy!(set_limit_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, x_spd_max, 0.0);
...
pub fn install() {
    Agent::new("donkey")
    .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_HI, status_donkey_SpecialHi_Main)
