...
unsafe extern "C" fn status_waluigi_SpecialFEnd_Pre(fighter: &mut L2CFighterCommon) -> L2CValue {
	...
	if WALUIGI && fighter_kind == FIGHTER_KIND_DOLLY {   
        ...
        original_status(Pre, fighter, *FIGHTER_DOLLY_STATUS_KIND_SPECIAL_F_END)(fighter)
    }
}

pub fn install() {
    Agent::new("dolly")
    .status(Pre, *FIGHTER_DOLLY_STATUS_KIND_SPECIAL_F_END, status_waluigi_SpecialFEnd_Pre)
    .install();
}