...
unsafe extern "C" fn status_simon_SpecialHi_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(simon_SpecialHi_Main_loop as *const () as _))
}

unsafe extern "C" fn simon_SpecialHi_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    Agent::new("simon")
    .status(Main, *FIGHTER_STATUS_KIND_SPECIAL_HI, status_simon_SpecialHi_Main)
    .install();
}