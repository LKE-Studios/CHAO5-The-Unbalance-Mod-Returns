...
unsafe extern "C" fn status_edge_SpecialHiEnd_Main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    sv_kinetic_energy!(set_brake, fighter, FIGHTER_KINETIC_ENERGY_ID_STOP, rush_end_brake_x.abs(), 0.0);
    fighter.clear_lua_stack();
    ...
    sv_kinetic_energy!(set_accel, fighter, FIGHTER_KINETIC_ENERGY_ID_GRAVITY, -rush_end_gravity_accel);
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(edge_SpecialHiEnd_Main_loop as *const () as _))
}

pub unsafe fn edge_special_hi_param_int_helper(fighter: &mut L2CFighterCommon, hash: L2CValue, charged_rush: L2CValue) -> L2CValue {
...
pub unsafe fn edge_special_hi_param_float_helper(fighter: &mut L2CFighterCommon, hash: L2CValue, charged_rush: L2CValue) -> L2CValue {
    let param = edge_special_hi_param_helper_inner(hash, charged_rush).get_u64();
    WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_hi"), param).into()
}

pub unsafe fn edge_special_hi_param_helper_inner(hash: L2CValue, charged_rush: L2CValue) -> L2CValue {
...
unsafe extern "C" fn edge_SpecialHiEnd_Main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        if !MotionModule::is_end(fighter.module_accessor) {
            ...
            if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_EDGE_STATUS_SPECIAL_HI_FLAG_ENABLE_CONTROL) {
                ...
                sv_kinetic_energy!(mul_x_speed_max, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, control_speed_x_max_mul);
                sv_kinetic_energy!(mul_x_accel_mul, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, control_accel_x_mul);
                sv_kinetic_energy!(mul_x_accel_add, fighter, FIGHTER_KINETIC_ENERGY_ID_CONTROL, control_accel_x_mul);
...
pub fn install() {
    Agent::new("edge")
    .status(Main, *FIGHTER_EDGE_STATUS_KIND_SPECIAL_HI_END, status_edge_SpecialHiEnd_Main)
    .install();
}