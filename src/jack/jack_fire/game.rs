...
//Fly
unsafe extern "C" fn game_jack_fire_Fly(fighter: &mut L2CAgentBase) {
...
//Hit
unsafe extern "C" fn game_jack_fire_Hit(fighter: &mut L2CAgentBase) {   
    if is_excute(fighter) {
...
pub fn install() {
    Agent::new("jack_fire")
    .game_acmd("game_fly", game_jack_fire_Fly, Low)
    .game_acmd("game_hit", game_jack_fire_Hit, Low)
    .install();
}