...
static ANGLE_MAX_UP : f32 = 65.0; //Max Upward Angle
static ANGLE_MAX_DOWN : f32 = -65.0; //Max Downward Angle
static mut MOMENTUM : [f32; 8] = [0.0; 8];
static ANGLE_MAX_SPEED : f32 = -20.0;
static BASE_H_SPEED : f32 = 1.9; //Base H Speed forward on glide
static GRAVITY : f32 = -0.44; //Default descent speed when unangled
static DOWN_H_SPEED : f32 = 2.52; //Sorta Emulates how Brawl handled gaining additional forward speed
static Y_ACCEL_ADD : f32 = 0.04; //Vertical Speed Multiplier
static X_DECEL_MUL_UP : f32 = -0.0213187103;
static X_ACCEL_MUL_DOWN : f32 = -0.025; 
static X_DECEL_MUL_DOWN : f32 = 0.0308572; 
static ANGLE_SPEED_MAX : f32 = 6.0; //Max Angular Speed
static mut UP_ANGLE_ACCEL_MIN : [f32; 8] = [0.7; 8]; //Min Upward angular acceleration 
static UP_ANGLE_ACCEL_MAX : f32 = 1.0; //Max Upward angular acceleration
static mut DOWN_ANGLE_ACCEL_MIN : [f32; 8] = [0.5; 8]; //Min Downward angular acceleration 
static DOWN_ANGLE_ACCEL_MAX : f32 = 0.9; //Max Downward angular acceleration
...
unsafe extern "C" fn glide_core(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if stick_y > 0.0 { //Used to prevent having a stick_y in the middle from changing flight angle
        ...
        if ANGLE[ENTRY_ID] > ANGLE_MAX_UP {
            ANGLE[ENTRY_ID] = ANGLE_MAX_UP; //Caps the max upward value at 65 and prevents it from going beyond. 
    ...
    if stick_y <= 0.0 {
        ...
        if ANGLE[ENTRY_ID] < ANGLE_MAX_DOWN {
            ANGLE[ENTRY_ID] = ANGLE_MAX_DOWN; //Caps the max downward value at -65 and prevents it from going beyond. 
            MOMENTUM[ENTRY_ID] = ANGLE_MAX_SPEED;
    ...
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN {
        ...
        SoundModule::set_se_pitch_ratio(fighter.module_accessor, Hash40::new("se_pit_landing02_win01"), 1.0 + ANGLE[ENTRY_ID] * -0.0047);
    }
    //Forward Speed Stuff
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_DOWN && ANGLE[ENTRY_ID] < ANGLE_MAX_SPEED { //Applies the H Air decel. multilplier when descending when angle is between -65 and -35.1
    ...
    if ANGLE[ENTRY_ID] >= ANGLE_MAX_SPEED && ANGLE[ENTRY_ID] < 0.0 { //Applies the H Air accel. multilplier when descending when angle is between -20 and 0.1
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    };
    if ANGLE[ENTRY_ID] <= ANGLE_MAX_UP && ANGLE[ENTRY_ID] > 0.0 { //Applies the H Air decel. multilplier when descending when angle is between 30.1 and 65
        macros::SET_SPEED_EX(fighter, BASE_H_SPEED + x, y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
