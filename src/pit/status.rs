...
unsafe extern "C" fn glide_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    macros::SET_SPEED_EX(fighter, 2.087, -0.357, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
    static Y_ACCEL_ADD : f32 = 0.0436;
    static X_ACCEL_ADD : f32 = 0.0;
    static X_DECEL : f32 = -0.00516; 
    ...
    if stick_y >= 0.1 || stick_y <= -0.1 { //Used to prevent having a stick_y in the middle from changing flight angle
        ...
        if ANGLE2[ENTRY_ID] < ANGLE_LOW_MAX2 {
            ANGLE2[ENTRY_ID] = ANGLE_LOW_MAX2;
        };
    };
    ...
    macros::SET_SPEED_EX(fighter, 2.087 + X[ENTRY_ID] + x, -0.357 + Y[ENTRY_ID] + y, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
